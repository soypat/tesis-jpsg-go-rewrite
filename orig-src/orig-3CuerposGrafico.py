...
def trayectoria():
    ...
    ### Jacobi grid para el fondo y trayectoria completa
    ...
    '''
    plt.plot(sol1.y[0], sol1.y[1], 'g-')
    plt.plot(sol2.y[0], sol2.y[1], 'r-')
    plt.plot(sol3.y[0], sol3.y[1], 'b-')
    plt.plot(sol4.y[0], sol4.y[1], 'm-')
    ...
    plt.colorbar(contours, label="Potencial de Jacobi [km²/s²]")

    plt.xlabel("x [km]", fontsize=fontForLabel)
    plt.ylabel("y [km]", fontsize=fontForLabel)
    plt.title("Transferencia Tierra-Luna",fontsize=fontForLabel)
    plt.legend()
    #plt.axis('equal')
    plt.grid(True)
